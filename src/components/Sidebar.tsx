...
const menuItems: { id: ViewType; label: string; icon: JSX.Element; group?: string; permission?: string }[] = [
  ...
  },
  {
    ...
    icon: (
      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732zM11 11l2 2m-4 4l4-4" />
      </svg>
    ),
  },
  {
    ...
    <aside className="w-56 bg-white border-r border-gray-200 flex flex-col">
      <nav className="flex-1 py-4 overflow-y-auto">
        {groupedItems.map((group, gi) => (
          <div key={gi} className="mb-2">
            {group.name && (
              <div className="px-6 pt-3 pb-1 text-xs font-semibold text-gray-400 uppercase tracking-wider">
                {group.name}
              </div>
            )}
      ...
      {selectedCell && (
        <div className="p-3 border-t border-gray-200">
          <div className="text-xs text-gray-500 mb-1">当前方格</div>
          <div className="text-sm font-medium text-gray-800">{selectedCell.code}</div>
